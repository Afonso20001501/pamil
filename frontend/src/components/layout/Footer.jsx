...
export default function Footer() {
  const { data: apiServices, loading: loadingServices, error: servicesError } = useApiData(
    api.getServices,
    [],
    []
  );
  ...
  const serviceList =
    !loadingServices && !servicesError && apiServices.length > 0 ? apiServices : mockServices;
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-4 gap-12">
        <div>
          ...
          <div className="flex gap-4 mt-6">
            <a href={info.instagram_url || '#'} aria-label="Instagram" className="hover:text-cue transition-colors"><Instagram size={18} /></a>
            <a href={info.facebook_url || '#'} aria-label="Facebook" className="hover:text-cue transition-colors"><Facebook size={18} /></a>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm">
            <li className="flex items-center gap-2">
              <Phone size={15} className="text-spotlight" /> {info.phone || '+244 929 818 191'}
            </li>
            <li className="flex items-center gap-2">
              <Mail size={15} className="text-spotlight" /> {info.email || 'contacto@pamilproducoes.ao'}
            </li>
            <li className="flex items-start gap-2">
              <MapPin size={15} className="text-spotlight mt-0.5" /> {info.address || 'Lubango, Angola'}
            </li>
          </ul>
        </div>
      </div>
