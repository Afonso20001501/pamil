...
const eventTypeCodes = [
  { code: 'corporativo', value: 'Corporativo', icon: Building2 },
  { code: 'show', value: 'Show/Concerto', icon: Music },
  { code: 'festival', value: 'Festival', icon: PartyPopper },
  { code: 'casamento', value: 'Casamento', icon: Heart },
  { code: 'privado', value: 'Evento Privado', icon: Sparkles },
  { code: 'outro', value: 'Outro', icon: HelpCircle },
];

const budgetCodes = ['ate_500k', '500k_2m', '2m_5m', 'acima_5m', 'a_definir'];
...
export default function PedidoOrcamento() {
  ...
  const steps = [
    t('quote.steps.eventType'),
    t('quote.steps.services'),
    t('quote.steps.details'),
    t('quote.steps.contact'),
  ];

  const eventTypeOptions = eventTypeCodes.map((e) => ({
    ...e,
    label: t(`quote.eventTypes.${e.code}`),
  }));

  const budgetOptions = budgetCodes.map((code) => ({
    value: code,
    label: t(`quote.budgetOptions.${code}`),
  }));
    ...
    return (
      <section className="min-h-[70vh] flex items-center justify-center bg-forest-dark px-6">
        <div className="text-center max-w-lg animate-riseIn">
          ...
          <span className="eyebrow">{t('quote.successEyebrow')}</span>
    ...
    <section className="bg-paper py-16 lg:py-20">
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-12">
          <span className="eyebrow !text-forest">{t('quote.eyebrow')}</span>
          ...
          {requestedArtist && (
            <p className="mt-4 inline-flex items-center gap-2 bg-cue/10 text-forest text-sm font-medium px-4 py-2 rounded-sm">
              🎤 {t('quote.requestingArtist')}: <strong>{requestedArtist}</strong>
            </p>
          )}
        </div>
          ...
          {steps.map((label, i) => (
            <div key={label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-2">
                <div
                  className={`h-9 w-9 rounded-full flex items-center justify-center font-mono text-xs border-2 transition-colors ${
                    i < step
                ...
                <span className={`text-[11px] font-mono uppercase tracking-widest2 hidden sm:block ${
                  i <= step ? 'text-forest' : 'text-ink/30'
                }`}>
                  {label}
                </span>
              </div>
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-[1.6fr_1fr] gap-10">
          <div className="bg-forest-dark rounded-sm p-8 lg:p-10 min-h-[420px]">
            ...
            {step === 0 && (
              <div className="animate-riseIn">
                <h2 className="font-body font-semibold text-2xl text-paper mb-6">
                  {t('quote.step1Question')}
                </h2>
                <div className="grid sm:grid-cols-3 gap-3">
                  {eventTypeOptions.map(({ value, label, icon: Icon }) => (
                    <button
                      type="button"
                      key={value}
            ...
            {step === 1 && (
              <div className="animate-riseIn">
                <h2 className="font-body font-semibold text-2xl text-paper mb-2">
                  {t('quote.step2Question')}
                </h2>
                <p className="text-paper/50 text-sm mb-6">{t('quote.step2Hint')}</p>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {availableServices.map((s) => (
                      <button
                        ...
                        className={`flex items-center gap-3 px-4 py-3.5 rounded-sm border text-left transition-all ${
                      ...
                      >
                        <span
                          className={`h-4 w-4 rounded-sm border flex items-center justify-center shrink-0 ${
                            form.services.includes(s.id) ? 'bg-cue border-cue' : 'border-paper/30'
                          }`}
                        >
            ...
            {step === 2 && (
              <div className="animate-riseIn space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <DarkField label={t('quote.eventDate')} type="date" value={form.event_date}
                    onChange={(v) => update('event_date', v)} />
                  <DarkField label={t('quote.guests')} type="number" value={form.guests_estimate}
                    onChange={(v) => update('guests_estimate', v)} />
                </div>
                <DarkField label={t('quote.location')} value={form.location}
            ...
            {step === 3 && (
              <div className="animate-riseIn space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <DarkField label={t('quote.fullName')} value={form.full_name}
                    onChange={(v) => update('full_name', v)} required />
                  <DarkField label={t('quote.email')} type="email" value={form.email}
                    onChange={(v) => update('email', v)} required />
                </div>
                <div className="grid sm:grid-cols-2 gap-5">
                  <DarkField label={t('quote.phone')} value={form.phone}
                    onChange={(v) => update('phone', v)} required />
                  <DarkField label={t('quote.company')} value={form.company}
                ...
                {status === 'error' && (
                  <p className="flex items-center gap-2 text-red-400 text-sm">
                    <AlertCircle size={16} /> {t('quote.sendError')}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="flex justify-between items-center mt-10 pt-6 border-t border-cue/10">
              ...
              >
                <ArrowLeft size={15} /> {t('quote.back')}
              </button>
              ...
              {step < steps.length - 1 ? (
                ...
                >
                  {t('quote.continue')} <ArrowRight size={16} />
                </button>
              ) : (
                <button
                  type="submit"
                ...
                >
                  {status === 'sending' ? (
                    <>
                      <Loader2 size={16} className="animate-spin" /> {t('quote.sending')}
                    </>
                  ) : (
                    <>
                      {t('quote.confirm')} <CheckCircle2 size={16} />
                    </>
                  )}
                </button>
              )}
          ...
          <aside className="bg-forest rounded-sm p-6 h-fit lg:sticky lg:top-24">
            ...
            <dl className="space-y-4 text-sm">
              {requestedArtist && <SummaryRow label={t('quote.summaryArtist')} value={requestedArtist} />}
              <SummaryRow label={t('quote.summaryType')} value={form.event_type} />
              <SummaryRow
                label={t('quote.summaryServices')}
                value={
              ...
              />
              <SummaryRow label={t('quote.summaryDate')} value={form.event_date} />
              <SummaryRow label={t('quote.summaryLocation')} value={form.location} />
              <SummaryRow
                label={t('quote.summaryBudget')}
                value={budgetOptions.find((b) => b.value === form.budget_range)?.label}
              />
              <SummaryRow label={t('quote.summaryName')} value={form.full_name} />
            </dl>
          </aside>
        </form>
      </div>
